...
async def _check_ollama(ollama_host: str) -> str:
    ...
    try:
        ...
        logger.error(f"Ollama health check failed: connection refused")
        return "unhealthy: connection refused"
    except httpx.TimeoutException:
        logger.error(f"Ollama health check failed: timeout")
...
@router.get(
    ...
    description="Health check simplificado (compatibility)",
    status_code=status.HTTP_200_OK
)
async def health() -> Dict[str, str]:
    ...
    return {
        "status": "healthy",
        "message": "Use /health/readiness para verificação completa"
    }
